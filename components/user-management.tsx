...
type Profile = { id: string; email: string; display_name: string | null; role: string; can_manage_matches: boolean; can_edit_other_player_stats: boolean; can_view_other_manager_stats: boolean; can_send_invites: boolean; can_manage_seasons: boolean; can_manage_permissions: boolean }
...
export function UserManagement({ profiles, currentUserId, isAdmin, canManagePermissions, canSendInvites }: { profiles: Profile[]; currentUserId: string; isAdmin: boolean; canManagePermissions: boolean; canSendInvites: boolean }) {
  ...
  return <>
    ...
    {canManagePermissions ? <section className="overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900"><div className="border-b border-zinc-800 px-5 py-4"><h2 className="text-lg font-semibold">Usuarios y Permisos</h2><p className="mt-1 text-sm text-zinc-400">Los permisos se aplican únicamente a este equipo. La cuenta de administrador conserva acceso completo.</p></div><div className="divide-y divide-zinc-800">{profiles.map((profile) => <UserPermissionRow key={profile.id} profile={profile} isAdmin={isAdmin} isSelf={profile.id === currentUserId} />)}</div></section> : <p className="text-sm text-zinc-400">No tienes permiso para administrar usuarios.</p>}
  </>
}


function UserPermissionRow({ profile, isAdmin, isSelf }: { profile: Profile; isAdmin: boolean; isSelf: boolean }) {
